...
rem    Studio? 2012 Professional Edition linker
rem
rem    $Revision: 1.1.6.1 $  $Date: 2012/09/25 18:19:50 $
rem
rem ********************************************************************
...
set PATH=%ICPP_COMPILER13%\Bin\ia32;%VCINSTALLDIR%\bin;%VCINSTALLDIR%\VCPackages;%VSINSTALLDIR%\Common7\IDE;%VSINSTALLDIR%\Common7\Tools;%LINKERDIR%\bin;%LINKERDIR%\bin\x86;%MATLAB_BIN%;%PATH%
set INCLUDE=%ICPP_COMPILER13%\compiler\Include;%VCINSTALLDIR%\INCLUDE;%VCINSTALLDIR%\ATLMFC\INCLUDE;%LINKERDIR%\include\um;%LINKERDIR%\include\shared;%LINKERDIR%\include\winrt;%INCLUDE%
set LIB=%ICPP_COMPILER13%\compiler\Lib\ia32;%VCINSTALLDIR%\LIB;%VCINSTALLDIR%\ATLMFC\LIB;%LINKERDIR%\lib;%LINKERDIR%\lib\win8\um\x86;%MATLAB%\extern\lib\win32;%LIB%
